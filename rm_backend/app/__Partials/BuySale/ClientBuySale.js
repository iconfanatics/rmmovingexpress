...
const marketValue = async (
  ...
  try {
    ...
    console.log( 
      ...
      // const perGroundPrice = purchaseValue / groundQty;

      let discountLandPrice

      if (groundType === 1) {
          if (promoDiscount > 0) {
              const discount = (promoDiscount / 100) * gOne;
              discountLandPrice = gOne - discount;
          } else {
              discountLandPrice = gOne;
          }
      }

      if (groundType === 2) {
          if (promoDiscount <= 0) {
              const discount = (promoDiscount / 100) * gTwo;
              discountLandPrice = gTwo - discount;
          } else {
              discountLandPrice = gTwo;
          }
      }
      console.log('discountLandPrice', discountLandPrice);
      

      const toTalMarketValue = Number(discountLandPrice) * groundQty;
      // const percentage =
      //     ((Number(discountLandPrice) - Number(perGroundPrice)) /
      //         Number(perGroundPrice)) *
      //     100;

      // if (purchaseValue) {
      //     return percentage; // Return as a formatted string
      // }
      return toTalMarketValue; // Return as a formatted string
  } catch (error) {
      console.error('Error in marketValue:', error);
      return 0; // Default fallback value
  }
};

/**
 * Function to get the billboard value all country
 * @param {String} countyCode
 * @param {String} ground_type
 * @returns {Array}
 */
// export const billboardMarketValue = (
//   gOne,
//   gTwo,
//   promoDiscount,
//   groundType
// ) => {
const billboardMarketValue = async (
    gOne,
    gTwo,
    promoDiscount,
    groundType
  ...
  const toTalMarketValue = Number(discountLandPrice);
...
const getLandValue = async (countyCode, ground_type) =>{
    try {
        ...
        if(countyCode){
            ...
            const groundVlaueOne = parseFloat(findLandValue.property_value_ground_one).toFixed(4);
            const groundVlaueTwo = parseFloat(findLandValue.property_value_ground_two).toFixed(4);
           
            // check promotion discount
            if (findLandValue.promotion_discount <= 0) {
                if (ground_type == '1') {
             ...
             // check without promotion discount
                ...
                if (ground_type == '1') {
                   landValue = calculateWithValueParcentage(groundVlaueOne, findLandValue.promotion_discount);
                } else if (ground_type == '2') {
                   landValue = calculateWithValueParcentage(groundVlaueTwo, findLandValue.promotion_discount);
                }
            }
        }
