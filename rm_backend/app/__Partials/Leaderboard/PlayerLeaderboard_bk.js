...
const playerTotalAssetValue = async (
  ...
  // Getting the CLIENTS
  var clients = await Client.query()
    // Countries
    ...
    .with('clientrobotics', (builder) => {
      builder
        ...
        .select('id', 'client_id', 'country_id', 'robotic_id', 'purchase_value')
    })
    // Resources
    ...
    .fetch()
  if (!clients) return []
  clients = clients ? clients.toJSON() : []
  ...
  return clients
  // return sortedArray.slice(0, topHowMany)
  // return clients
}
/**
...
const playerExpensiveGroundPurchase = async (
  ...
  clients = clients.sort((a, b) => {
    ...
    // return maxGroundValueA - maxGroundValueB
  })
  // returning based on params
  const topHowMany = filter_by_top_position
    ? parseInt(filter_by_top_position.match(/\d+/))
