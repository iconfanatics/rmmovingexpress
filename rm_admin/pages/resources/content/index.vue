  ...
  <client-only>
    <div>
      ...
      >
        ...
        <!-- End of Close Button -->
        <div class="de-drawer-header">Add Resources Content</div>
        <v-divider></v-divider>
...
<script>
import {
  addNewResourceContentUrl,
  allServiceNameUrl,
  changeResourceContentStatusUrl,
  deleteResourceContentUrl,
  getAllResourceContentUrl,
  updateResourceContentUrl
...
export default {
  ...
  computed: {
     ...
     */
    isValidToAddFaq() {
      if (this.newFaq.title && this.newFaq.sub_title) {
        return true
      } else {
        return false
      }
     ...
     */
    isValidToUpdateFaq() {
      if (this.editFaq.id && this.editFaq.title && this.editFaq.sub_title) {
        return true
      } else {
        return false
      }
  ...
  methods: {
    ...
    async addNewFaq() {
      ...
      try {
        ...
        await _this.$axios
          .$post(addNewResourceContentUrl, {
            resoruceContent: _this.newFaq
          })
          .then((response) => {
     ...
     */
    async changeFaqStatus(resourceContentId) {
      let _this = this
      try {
        _this.$swal
          .fire({
            ...
            preConfirm: (isConfirm) => {
              return _this.$axios
                .$post(changeResourceContentStatusUrl, {
    ...
    async updateFaq() {
      ...
      try {
        ...
        await _this.$axios
          .$post(updateResourceContentUrl, {
            editResourceContentInfo: _this.editFaq
          })
          .then((response) => {
    ...
    async deleteFaq(id) {
      try {
        ...
        await _this.$axios
          .$delete(`${deleteResourceContentUrl}/${id}`)
          .then((response) => {
            ...
            if (response && response.STATUS === 400) {
              ...
              this.faqData.splice(
                this.faqData
                  ...
                  .indexOf(response.resourceContentInfo.id),
                1
              )
              _this.$toast.success(
                response.MESSAGE,
